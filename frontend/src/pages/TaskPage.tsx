...
export default function TaskPage() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://13.232.144.164:5051/u1/api/tasks/view', {
        headers: {
  ...
  const updateTaskStatus = async (taskId: string, newStatus: 'todo' | 'in-progress' | 'done') => {
    try {
      await axios.put(`http://13.232.144.164:5051/u1/api/tasks/update/${taskId}`, 
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`
  ...
  const handleDelete = async (taskId: string) => {
    try {
      await axios.delete(`http://13.232.144.164:5051/u1/api/tasks/delete/${taskId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
